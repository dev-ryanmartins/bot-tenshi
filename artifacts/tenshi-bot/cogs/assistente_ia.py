...
class AssistenteIA:
    ...
    async def _responder(self, message, texto: str):
        ...
        try:
            ...
            bot_logger.error(f"Erro no assistente de IA ao responder: {exc}", exc=exc)
            embed = discord.Embed(
                title="Tenshi responde",
                description="*Perdoe-me, nobre habitante. Uma névoa impediu minha clarividência neste instante. Poderia reformular seu pedido?*",
                color=COR_NEUTRO,
            )
            embed.set_footer(text=RODAPE_IMPERIAL)
            try:
