...
# Constantes de cor para compatibilidade visual imperial
COR_DOURADO = CoresImperial.DOURADO
COR_SUCESSO = CoresImperial.SUCESSO
COR_PERIGO = CoresImperial.PERIGO
COR_INFO = CoresImperial.INFO
COR_NEUTRO = CoresImperial.NEUTRO
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            try:
                ...
                if getattr(he, 'code', None) == 30005:
                    await interaction.followup.send("⚠️ O servidor atingiu o limite máximo de 250 cargos do Discord. Remova alguns cargos antigos do servidor para criar novos.", ephemeral=True)
                else:
                    await interaction.followup.send(f"❌ Erro ao criar cargo no Discord: {he.text if hasattr(he, 'text') else he}", ephemeral=True)
        except Exception as e:
            await interaction.followup.send(f"❌ Erro: {str(e)}", ephemeral=True)
