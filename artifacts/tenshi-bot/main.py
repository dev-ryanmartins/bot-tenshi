...
def sanitizar_discord_token(raw_token: str | None) -> str | None:
    """Limpa e valida superficialmente um token do Discord para evitar LoginFailure por formatação."""
    if not raw_token:
        return None
    token = str(raw_token).strip()
    # Remove aspas externas simples, duplas ou crases que usuários costumam colar por engano
    token = token.strip('"\'` \t\r\n')
    # Remove prefixo 'Bot ' ou 'Bearer ' se colado junto
    ...
    # Placeholders comuns
    placeholders = {
        "your_token_here", "your_discord_token", "discord_token", "seu_token_aqui",
        "seu_token", "token_aqui", "token", "none", "null", "undefined", "xxx", "insert_token_here"
    }
    if token.lower() in placeholders or len(token) < 20:
        return None
    return token


def _carregar_env_local():
    """Carrega variáveis de arquivos .env caso ainda não definidas e sanitiza variáveis de ambiente."""
    possiveis = [
        os.path.join(os.getcwd(), ".env"),
        os.path.join(os.path.dirname(__file__), ".env"),
        os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), ".env"),
    ]
    for env_path in possiveis:
        ...
        try:
            with open(env_path, "r", encoding="utf-8") as f:
                for raw_line in f:
                    ...
                    var_name = name.strip()
                    var_val = value.strip().strip('"\'` \t\r\n')
                    if var_val and var_name not in os.environ:
                        os.environ[var_name] = var_val
        except Exception:
            pass

    # Sanitiza DISCORD_TOKEN vindo do ambiente do Railway caso tenha sido colado com aspas
    if "DISCORD_TOKEN" in os.environ:
        token_limpo = sanitizar_discord_token(os.environ.get("DISCORD_TOKEN"))
        if token_limpo:
            os.environ["DISCORD_TOKEN"] = token_limpo
...
def _salvar_status_bot(online: bool, mensagem: str | None = None):
...
async def on_ready():
    ...
    bot_logger.info(
        f"Bot Tenshi v2 online | {bot.user.name} ({bot.user.id}) | Servidores: {len(bot.guilds)} | Imperador: {IMPERADOR_ID}"
    )
    ...
    if not _bg_tasks_initialized:
        ...
        for guild in bot.guilds:
            ...
            if imperador:
                try:
                    ...
                    bot_logger.warning(f"Cargos do fundador não aplicados em {guild.name}: {exc}")
        ...
        try:
            ...
            bot_logger.info(f"{len(sincronizados)} comandos de barra sincronizados.")
            print(f"✅ {len(sincronizados)} comandos de barra sincronizados.")
        except Exception as exc:
            bot_logger.warning(f"Não foi possível sincronizar comandos de barra: {exc}")
        ...
        bot_logger.info("Tarefas de background inicializadas com sucesso.")
...
async def on_message(message):
    ...
    if not eh_comando:
        ...
        # Verificação ativa de palavras proibidas pelo filtro de moderação
        try:
            if await moderacao.verificar_palavras_proibidas(message):
                return
        except Exception as exc:
            bot_logger.error(f"Erro ao verificar palavras proibidas: {exc}", exc=exc)
    ...
    elif cmd in ("ban", "banir"):
        await moderacao.handle_ban(message, args)
        return

    elif cmd in ("kick", "expulsar"):
        await moderacao.handle_kick(message, args)
        return

    elif cmd in ("mute", "silenciar", "timeout"):
    ...
    elif cmd in ("proibir-palavra", "proibirpalavra", "add-palavra", "banir-palavra"):
        await moderacao.handle_proibir_palavra(message, args)
        return

    elif cmd in ("liberar-palavra", "liberarpalavra", "remover-palavra", "desproibir-palavra"):
        await moderacao.handle_liberar_palavra(message, args)
        return

    elif cmd in ("palavras-proibidas", "palavrasproibidas", "listar-palavras-proibidas", "palavras-bloqueadas"):
        await moderacao.handle_palavras_proibidas(message, args)
        return
    ...
    elif cmd in ("aviso", "warn", "advertencia", "advertência"):
        await moderacao.handle_aviso(message, args)
        return
    ...
    # ── STATUS DOS MOTORES DE IA ───────────────────────────────────────────────
        ...
        for key, info in motores.items():
            icone = "🟢" if info.get("ativo") else "🔴"
            linhas.append(f"{icone} **{key}** — `{info.get('modelo')}`")
    ...
    # 🔮 Perguntas Gerais para a IA Imperial (Exclusivo OpenRouter)
    elif cmd in ("perguntar", "ia", "pergunta", "tenshi-ia"):
        if not args:
            await message.channel.send(embed=embed_imperial(
                "🔮 Oráculo Imperial",
                "Envie uma questão, conversa ou consulta para a sabedoria da IA imperial.\n\n"
                "**Uso:** `Tenshi, oraculo <sua pergunta>`\n"
                "**Exemplo:** `Tenshi, oraculo como vai o reino?`",
                COR_OURO
            ))
            return
        prompt_usuario = " ".join(args)
        async with message.channel.typing():
            from ia_router import chamar_ia
            t_ini = time.perf_counter()
            resposta = await chamar_ia(
                sistema="Você é Tenshi, a inteligência imperial e soberana do Império Tenshi. "
                        "Responda com autoridade imperial, erudição e carisma à corte.",
                usuario=prompt_usuario,
                modelo="narrativa",
                max_tokens=1000,
                temperature=0.75
            )
            t_ms = round((time.perf_counter() - t_ini) * 1000)
            embed = discord.Embed(
                title="🔮 O ORÁCULO DE TENSHI RESPONDE",
                description=f"*\"{prompt_usuario[:250]}\"*\n{SEP}\n\n{resposta[:3800]}",
                color=0x4B0082
            )
            embed.set_footer(text=f"⚙️ OpenRouter  •  {t_ms}ms  •  {RODAPE_IMPERIAL}")
            await message.channel.send(embed=embed)
        return

    # 🌐 Consulta Direta ao OpenRouter
    elif cmd in ("openrouter", "ia-livre", "consulta-ia"):
        if not args:
            await message.channel.send(embed=embed_imperial(
                "🌐 OpenRouter — IA Imperial",
                "Consulte os motores neurais soberanos do Império Tenshi.\n\n"
                "**Uso:** `Tenshi, openrouter <sua pergunta>`",
                0x7C3AED
            ))
            return
        prompt_usuario = " ".join(args)
        async with message.channel.typing():
            from ia_router import chamar_openrouter
            t_ini = time.perf_counter()
            resposta = await chamar_openrouter(
                sistema="Você é a inteligência imperial de Tenshi.",
                usuario=prompt_usuario,
            ...
            t_ms = round((time.perf_counter() - t_ini) * 1000)
            embed = discord.Embed(
                title="🌐 OpenRouter Imperial",
                description=f"**Consulta:** *\"{prompt_usuario[:250]}\"*\n{SEP}\n\n{resposta[:3800]}",
                color=0x7C3AED
            )
            embed.set_footer(text=f"⚙️ OpenRouter • {t_ms}ms • {RODAPE_IMPERIAL}")
            await message.channel.send(embed=embed)
    ...
    # 📜 Redação Imperial de Decretos com IA
    elif cmd in ("decretar-ia", "decretar_ia", "redigir-decreto"):
        if not args:
            await message.channel.send(embed=embed_imperial(
                "📜 Redator Imperial de Decretos",
                "Redige um decreto formal solene em nome de Sua Majestade Imperador Alloy.\n\n"
                "**Uso:** `Tenshi, decretar-ia <tema>`\n"
                "**Exemplo:** `Tenshi, decretar-ia instituição de tributação sobre mercadorias contrabandeadas`",
                COR_OURO
            ))
            return
        tema = " ".join(args)
        async with message.channel.typing():
            ...
            t_ini = time.perf_counter()
            sistema_decreto = (
                "Você é o Alto Chanceler Jurídico do Império Tenshi. Redija um decreto imperial oficial solene "
                "em nome de Sua Majestade Imperial Imperador Alloy. O texto deve conter:\n"
                "1. Título do Decreto (ex: DECRETO SOBERANO IMPERIAL Nº ...)\n"
                "2. Preâmbulo formal e fundamentação jurídica imperial\n"
                "3. Artigos de lei detalhados (Art. 1º, Art. 2º...)\n"
                "4. Penalidades e punições aos infratores\n"
                "5. Selo imperial e promulgação oficial."
            )
            decreto_texto = await ia_soberana(sistema_decreto, f"Assunto: {tema}", max_tokens=1100)
            t_ms = round((time.perf_counter() - t_ini) * 1000)
            embed = discord.Embed(
                title="👑 DECRETO SOBERANO IMPERIAL",
                description=f"{decreto_texto[:3900]}",
                color=0xC5A059
            )
            embed.set_footer(text=f"📜 Chancelaria de Tenshi • {t_ms}ms • {RODAPE_IMPERIAL}")
            await message.channel.send(embed=embed)
        return

    # ✨ Profecias e Augúrios Narrativos para RPG
    elif cmd in ("profecia", "augurio", "augúrio", "oraculo-destino"):
        alvo = " ".join(args) if args else f"o destino de {message.author.display_name}"
        async with message.channel.typing():
            from ia_router import ia_narrativa
            t_ini = time.perf_counter()
            sistema_profecia = (
                "Você é a Grande Oráculo de Tenshi, guardiã dos astros etéreas. "
                "Crie uma profecia mística, poética e misteriosa para o destino da pessoa ou evento solicitado. "
                "Use linguagem nobre e de alta fantasia."
            )
            profecia_texto = await ia_narrativa(sistema_profecia, f"Profecia para: {alvo}", max_tokens=700)
            t_ms = round((time.perf_counter() - t_ini) * 1000)
            embed = discord.Embed(
                title="✨ Profecia dos Céus de Tenshi",
                description=f"**Destinado a:** *{alvo[:200]}*\n{SEP}\n\n{profecia_texto[:3800]}",
                color=0x4A154B
            )
            embed.set_footer(text=f"🌌 Astros Alinhados • {t_ms}ms • {RODAPE_IMPERIAL}")
            await message.channel.send(embed=embed)
        return

    # 🚀 Diagnóstico Completo de Deploy para GitHub / Railway / Render
    elif cmd in ("redeploy-check", "diagnostico-deploy", "status-deploy", "check-deploy"):
        import platform
        has_token = bool(os.environ.get("DISCORD_TOKEN"))
        has_gemini = bool(os.environ.get("GEMINI_API_KEY") or os.environ.get("GOOGLE_API_KEY"))
        has_openai = bool(os.environ.get("OPENAI_API_KEY"))
        has_openrouter = bool(os.environ.get("OPENROUTER_API_KEY"))
        db_path = os.path.exists("data/db.json") or os.path.exists("data/tenshi.db")
        lat = round(bot.latency * 1000) if bot.latency and not (bot.latency != bot.latency) else 0

        embed = discord.Embed(
            title="🚀 DIAGNÓSTICO DE DEPLOY IMPERIAL",
            description=(
                f"*Status de prontidão para GitHub, Railway e nuvem.*\n{SEP}\n\n"
                f"**🤖 Discord Gateway:** {'🟢 Conectado' if bot.is_ready() else '🟡 Conectando'} (`{lat}ms`)\n"
                f"**🔑 DISCORD_TOKEN:** {'🟢 Configurado' if has_token else '🔴 Ausente nas variáveis'}\n"
                f"**✨ Google Gemini API:** {'🟢 Ativa' if has_gemini else '⚪ Opcional (.env)'}\n"
                f"**🧠 OpenAI API:** {'🟢 Ativa' if has_openai else '⚪ Opcional (.env)'}\n"
                f"**⚡ OpenRouter API:** {'🟢 Ativa' if has_openrouter else '⚪ Opcional (.env)'}\n"
                f"**💾 Banco de Dados:** {'🟢 Operacional' if db_path else '🟡 Em inicialização'}\n"
                f"**🌐 Servidor Web / Health:** 🟢 Ativo na porta `8081 / $PORT`\n"
                f"**🐍 Ambiente Python:** `{platform.python_version()} ({platform.system()})`\n\n"
                f"{SEP}\n"
                f"✅ **Pronto para Redeploy:** As alterações sincronizadas no GitHub são aplicadas "
                f"imediatamente no próximo deploy sem conflitos."
            ),
            color=0x00A86B
        )
        embed.set_footer(text=f"Deploy Health Check  •  {RODAPE_IMPERIAL}")
        await message.channel.send(embed=embed)
        return
...
async def on_member_join(member):
    try:
        ...
        print(f"[AVISO] Permissão insuficiente para aplicar cargo inicial a {member}.")
    except discord.HTTPException as exc:
        if getattr(exc, "code", None) == 30005 or "30005" in str(exc) or "Maximum number of server roles reached" in str(exc):
            print(f"[AVISO] Servidor '{member.guild.name}' atingiu o limite de 250 cargos do Discord ao receber {member}. Registro concluído sem criação de novos cargos.")
        else:
            print(f"[AVISO] Falha ao preparar parentesco de {member}: {exc}")
    except Exception as exc:
        print(f"[AVISO] Erro inesperado ao receber {member}: {exc}")
...
async def on_error(event, *args, **kwargs):
    exc = sys.exc_info()[1]
    bot_logger.error(f"Exceção não tratada no evento Discord '{event}': {exc}", exc=exc)


def loop_execucao_resiliente():
    """Mantém o servidor web HTTP ativo na porta do Railway e orquestra a conexão do bot sem jamais crashar."""
    # Inicia a thread do servidor HTTP na porta do Railway imediatamente
    start_site_server_thread(bot)
    
    ultimo_token_rejeitado = None
    
    while True:
        _carregar_env_local()
        raw_token = os.environ.get("DISCORD_TOKEN")
        token = sanitizar_discord_token(raw_token)
        porta = os.environ.get("PORT") or os.environ.get("SITE_PORT") or "8081"
        
        if not token:
            _salvar_status_bot(False, mensagem="Aguardando DISCORD_TOKEN nas variáveis do Railway")
            print("=" * 70)
            print(f"🏛️ [TENSHI IMPERIAL] Deploy no Railway ativo! Servidor HTTP na porta {porta}")
            print("🌐 Healthcheck respondendo 200 OK em /health para manter deploy verde.")
            print("⚠️ [AVISO] DISCORD_TOKEN não configurado ou é um placeholder.")
            print("")
            print("👉 COMO ATIVAR SEU BOT NO RAILWAY EM 3 PASSOS:")
            print("   1. Acesse seu projeto no Railway -> Clique na aba 'Variables'")
            print("   2. Adicione a variável: DISCORD_TOKEN")
            print("   3. Cole o token do seu bot (NÃO use aspas nem adicione 'Bot ' antes)")
            print("   Assim que salvar, o bot conectará automaticamente sem precisar de redeploy!")
            print("=" * 70)
            
            while not token:
                time.sleep(10)
                _carregar_env_local()
                token = sanitizar_discord_token(os.environ.get("DISCORD_TOKEN"))
                if token:
                    print("⚡ [TENSHI IMPERIAL] Novo DISCORD_TOKEN detectado! Conectando ao Discord...")
                    ultimo_token_rejeitado = None
                    break
        
        # Se o token atual for o mesmo que já foi rejeitado pelo Discord com LoginFailure
        if token and token == ultimo_token_rejeitado:
            time.sleep(15)
            continue
            
        try:
            mascara = f"{token[:6]}...{token[-4:]}" if len(token) > 10 else "***"
            print(f"⚡ [TENSHI IMPERIAL] Iniciando conexão com o Discord (token: {mascara})...")
            bot.run(token)
            print("ℹ️ [TENSHI IMPERIAL] Sessão do bot finalizada. Verificando estado...")
            ultimo_token_rejeitado = None
            time.sleep(5)
        except discord.LoginFailure as exc:
            ultimo_token_rejeitado = token
            _salvar_status_bot(False, mensagem="Token rejeitado pelo Discord (LoginFailure)")
            print("=" * 70)
            print("❌ [ERRO CRÍTICO] DISCORD_TOKEN REJEITADO PELO DISCORD (LoginFailure)!")
            print(f"Detalhes retornados: {exc}")
            print("")
            print("💡 O Discord recusou o token porque ele é inválido, expirou ou foi resetado.")
            print("")
            print("👉 PASSO A PASSO PARA CORRIGIR NO RAILWAY:")
            print("   1. Abra o Discord Developer Portal: https://discord.com/developers/applications")
            print("   2. Selecione sua aplicação -> Clique no menu 'Bot' à esquerda")
            print("   3. Em 'Token', clique em 'Reset Token' e confirme")
            print("   4. Clique em 'Copy' para copiar o token novo")
            print("   5. No Railway -> seu serviço bot-tenshi -> aba 'Variables'")
            print("   6. Atualize o valor de DISCORD_TOKEN colando o token direto (sem aspas)")
            print("   7. Salve as alterações!")
            print("")
            print("🛡️ [RAILWAY SHIELD] O servidor web continua respondendo em /health;")
            print("   o container NÃO irá crashar e aguardará o novo token.")
            print("=" * 70)
            time.sleep(20)
        except Exception as exc:
            _salvar_status_bot(False, mensagem=f"Erro de conexão: {exc}")
            msg = str(exc)
            if "Cannot connect to host discord.com:443" in msg or "Acesso negado" in msg or "ClientConnectorError" in msg:
                print(f"[REDE DISCORD] Bloqueio ou falha de rota com discord.com:443 ({exc}). Tentando em 15s...")
            else:
                print(f"[ERRO NO BOT] Falha na execução: {exc}. Reiniciando conexão em 15s...")
            time.sleep(15)
        finally:
            _salvar_status_bot(False)
