...
**🧠 Inteligência Artificial & Oráculos (OpenRouter)**
`status-ia` *(status em tempo real dos 8 motores de IA no OpenRouter)*
`oraculo [pergunta]` *(consulta de alta sabedoria à corte imperial)*
`openrouter [pergunta]` *(consulta direta aos motores neurais)*
`decretar-ia [tema]` *(redação de decreto soberano oficial)*
`profecia [alvo/tema]` *(augúrio narrativo para RPG)*

**🚀 Deploy & Infraestrutura**
`redeploy-check` *(diagnóstico completo de prontidão para GitHub e Railway)*
{SEP}
