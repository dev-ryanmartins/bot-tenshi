...
def _bot_status(bot=None) -> dict[str, Any]:
    ...
    if not msg:
        msg = "Bot aguardando conexão com Discord..." if not data.get("online") else "Operacional"
...
def _data_overview() -> dict[str, Any]:
    ...
    status = _bot_status()
    has_openrouter = bool(os.environ.get("OPENROUTER_API_KEY")) or True
    return {
...
def create_app(bot=None) -> web.Application:
    ...
    async def health(_: web.Request) -> web.Response:
        ...
        return web.json_response({
            "ok": True,
            "service": "tenshi-python-site",
            "bot_connected": st.get("online", False),
            "status": "online" if st.get("online") else "standby",
            "message": st.get("message", "Operacional"),
        })
